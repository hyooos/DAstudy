...
| 날짜 | 데이터 | 발제자 | PR 마감 |
| --- | --- | --- | --- |
| 11.07 (토) | Bank Marketing | 형지원 (`tired-gini`) | 11.11 (수) 23:59 |

> **데이터가 바뀌는 주차입니다.**
> 다시 Bank Marketing을 사용합니다. 4주차에 만든 `common/bank/preprocess.py`의 `add_prev_contact_flag()`를 적용한 데이터로 모델링해주세요.

## 데이터 선정 이유

`duration`이라는, 데이터 제공처가 공식적으로 명시한 누수 변수가 있고, 행이 시간순이라 무작위 분할과 시간순 분할의 차이가 크게 드러납니다.
Home Credit은 신청 날짜가 없어서 시간순 검증을 할 수 없기 때문에 이 주제는 Bank가 가장 적합합니다. 1주차에 정한 예측 시점(D-001)을 여기서 검증합니다.
...
- `common/evaluate.py` — `make_pipeline()`, `time_split()` (8주차 스터디 전까지 merge)
